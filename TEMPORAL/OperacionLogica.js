...
function Logica(expresion, entorno, errores){
    if(expresion.tipo === TIPO_VALOR.INT || expresion.tipo === TIPO_VALOR.DOUBLE || 
        ...
        expresion.tipo === TIPO_OPERACION.TERNARIO){
        return Relacional(expresion, entorno, errores)
    }
...
function and(_izquierda, _derecha, entorno, errores){
    const izquierda = Logica(_izquierda, entorno, errores)
    const derecha = Logica(_derecha, entorno, errores)
